...
Rules:
- "title": a short, professional title for the overall expense report (e.g. "Gas Expense Reimbursement - Field Visits, June 2026").
...
- For each receipt: "receiptTitle" is a short, generic label for the receipt/photo itself (e.g. "Receipt 1", "Fuel Receipt 2") — do not invent a specific gas station brand or name, and do not include a price or dollar amount anywhere. "expenseTitle" is a short title describing this receipt as a line item in the expense report (e.g. "Fuel - Route to Store Visit"), also with no brand name or dollar amount. "note" is a longer, detailed paragraph (3-5 sentences) explaining that specific fill-up: why it was needed, what work it supported, and any relevant detail implied by the per-receipt input.
- Do not invent employee names, company names, dates, or dollar amounts.
...
function buildExpenseUserContent({ prompt, receiptCount, receiptPrompts, today }) {
  ...
  const receiptLines = Array.from({ length: count }, (_, i) => {
    ...
    return `Receipt ${i + 1}: ${note || '(no extra note given, use your best judgement)'}`;
  }).join('\n');

  return `Today's date: ${todayStr} (use this year/month for the report title/details unless the prompt below specifies a different time period)
