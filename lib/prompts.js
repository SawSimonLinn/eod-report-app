...
- Write "businessPurpose", "comment", and every receipt "note" in the first person, as if the employee themself is filling out and submitting this expense report (use "I", "my", not "the employee"). Do not write in a third-person, corporate, or policy-manual tone (avoid phrasing like "the employee performs their duties" or "this transportation is essential for fulfilling job requirements").
- "businessPurpose": one clear sentence, in the employee's own voice, stating why they needed gas for this trip to work.
- "comment": two to three concise sentences, in the employee's own voice, giving a bit more context (what the day/trip was for, how it relates to their job).
- If the "Overall report context/prompt" below is marked as not given by the user (no extra detail beyond the default), keep "businessPurpose" and "comment" very short and simple — just something like gas reimbursement for the regular daily commute to the assigned store. Do not pad it out with invented detail about the job (cutting fruit, training, region size, etc.) just because that background is available to you; that background is only for you to understand the situation, not material to describe in the output. Only go into more detail in "businessPurpose"/"comment" if the user actually typed their own context/prompt.
- "receipts" must contain EXACTLY the number of entries requested, in the same order as the per-receipt notes given.
- For each receipt: "receiptTitle" is a short, generic label for the receipt/photo itself (e.g. "Receipt 1", "Fuel Receipt 2") — do not invent a specific gas station brand or name. "expenseTitle" is a short title describing this receipt as a line item in the expense report (e.g. "Fuel - Route to Store Visit"), also with no brand name. "note" is a longer, detailed paragraph (3-5 sentences), in the employee's own first-person voice, explaining that specific fill-up: why I needed it, what work it supported, and any relevant detail implied by the per-receipt input.
...
const DEFAULT_EXPENSE_CONTEXT = `The employee is an STO (Short Term Operator) working for a fresh-cut fruit and vegetable business. The work is 6 days a week: cutting fruit and vegetables and preparing them for sale to customers in-store, day to day, the same core job each shift, working alongside 2-3 other people at their assigned store. This is not a training role — the employee does not train new hires or set up new stores. The gas expense is simply for the daily work commute: driving from home to the assigned store and back for the shift. Do not describe this as meeting clients, sales calls, training, or anything unrelated to the daily commute to work in-store.`;
...
function buildExpenseUserContent({ prompt, receiptCount, receiptPrompts, receiptImages, today }) {
  ...
  const userProvidedPrompt = (prompt || '').trim();
...
Overall report context/prompt: ${userProvidedPrompt || DEFAULT_EXPENSE_CONTEXT}${userProvidedPrompt ? '' : '\n(Note: the user did NOT type their own context/prompt — the line above is just background for you to understand the job. Keep "businessPurpose" and "comment" very short and simple, per the rules.)'}
